...
def update_segmento(id: str, req: SegmentoRedUpdate, db: Session = Depends(get_db)):
    segmento = db.query(SegmentoRed).filter(SegmentoRed.id == id).first()
    if not segmento:
        raise HTTPException(status_code=404, detail="Segmento no encontrado")
...
def delete_segmento(id: str, db: Session = Depends(get_db)):
    segmento = db.query(SegmentoRed).filter(SegmentoRed.id == id).first()
    if not segmento:
        raise HTTPException(status_code=404, detail="Segmento no encontrado")
    db.delete(segmento)
...
def create_ip(req: DireccionIpAsignadaCreate, db: Session = Depends(get_db)):
    ...
    # Validacion estricta IP pertenece al Segmento
...
def update_ip(id: str, req: DireccionIpAsignadaUpdate, db: Session = Depends(get_db)):
    ...
    # Si cambian la IP o el segmento, validar de nuevo
...
def delete_ip(id: str, db: Session = Depends(get_db)):
    ip_obj = db.query(DireccionIpAsignada).filter(DireccionIpAsignada.id == id).first()
    if not ip_obj:
        raise HTTPException(status_code=404, detail="IP no encontrada")
    db.delete(ip_obj)
