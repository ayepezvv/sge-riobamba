...
# Make sure we catch any previous state of the function
...
content = re.sub(r'@router.get\("/plantillas/\{id\}/esquema"\).*?(?:raise HTTPException\(status_code=500, detail=f"Error procesando documento: \{str\(e\)\}"\)|raise HTTPException\(500, f"Error analizando plantilla con jinja2: \{str\(e\)\}"\))', new_func, content, flags=re.DOTALL)
