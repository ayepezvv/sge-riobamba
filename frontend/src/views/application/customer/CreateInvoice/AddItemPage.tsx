...
export default function AddItemPage({ handleAddItem, setAddItemClicked }: any) {
  ...
  const itemList = [
    { id: 111, name: 'Product Name 1', amount: 260, desc: 'Product Description 1' },
    { id: 112, name: 'Product Name 2', amount: 200, desc: 'Product Description 2' },
    { id: 113, name: 'Product Name 3', amount: 300, desc: 'Product Description 3' }
  ];
  ...
  const handleChange = (event: any) => {
    ...
    if (event.target.name === 'quantity') {
      if (Number(value) < 0) {
        setErrors({ ...errors, quantityError: 'negative values not allowed' });
        setSelectedQuantity(value);
      } else if (Number(value) === 0) {
        setErrors({ ...errors, quantityError: 'quantity can not be zero' });
        setSelectedQuantity(value);
      } else {
        setSelectedQuantity(value);
  ...
  const handleOk = () => {
    const data = { ...selectedItem, totalAmount: amount, selectedQuantity };
    handleAddItem(data);
  };
