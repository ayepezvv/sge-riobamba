...
export default function ParsingValues() {
  ...
  const { data } = useDemoData({ dataSet: 'Employee', visibleFields: VISIBLE_FIELDS, rowLength: 100 });
      ...
      data.columns
        ...
        .map((column) => {
          if (column.field === 'dateCreated') {
            return { ...column, getApplyQuickFilterFn: getApplyQuickFilterFnSameYear };
          }
          if (column.field === 'name') {
            return { ...column, getApplyQuickFilterFn: undefined };
          }
          return column;
        }),
    [data.columns]
      ...
      <Box sx={{ width: '100%', ...dataGridStyles }}>
        <DataGrid
          ...
          initialState={{
            ...
            filter: { ...data.initialState?.filter, filterModel: { items: [], quickFilterLogicOperator: GridLogicOperator.Or } }
          }}
